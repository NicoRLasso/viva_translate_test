...
from viva_translate.teams.serializers import TeamSerializer,AddMemberTeamSerializer
...
class TeamsViewSet(ModelViewSet):
    ...
    @action(
            detail=True, 
            methods=['get'],
            url_path=r'add_member/(?P<member_pk>\w+)',
            url_name='add_member')
    def add_member(self, request, pk=None, member_pk=None):
        serializer = AddMemberTeamSerializer(data={'team_id':pk,'member_id':member_pk})
        if serializer.is_valid(raise_exception=True):
            team = Teams.objects.get(pk=serializer.validated_data['team_id'])
            if team.members.count()<3:
                participant =Participants.objects.get(pk=serializer.validated_data['member_id'])
                ...
                return Response({"Message":f"All members must be from {team.country.name}"})
    ...
    def remove_member(self, request, pk=None,member_pk=None):
        ...
        if serializer.is_valid(raise_exception=True):
            team = Teams.objects.get(pk=serializer.validated_data['team_id'])
            team.members.remove(Participants.objects.get(pk=serializer.validated_data['member_id']))
            serializer_class = TeamSerializer(team)
            return Response(data=serializer_class.data, status=status.HTTP_202_ACCEPTED)
